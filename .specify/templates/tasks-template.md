...
## Path Conventions (Loretide - this repo, not generic examples)

- **Backend**: `server/` (Go, Chi router, sqlc, gorilla/websocket)
- **Web**: `apps/web/` (Next.js App Router); Next.js-only APIs live in `apps/web/platform/`
- **Desktop**: `apps/desktop/`; **Mobile**: `apps/mobile/` (independent - read `apps/mobile/CLAUDE.md` first)
- **Docs site**: `apps/docs/`
- **Shared headless logic**: `packages/core/`; **atomic UI**: `packages/ui/`; **shared business views**: `packages/views/`
- Dependency direction is `views -> core + ui`; `core` and `ui` stay independent.
- There is no `src/`, `backend/`, or `frontend/` directory in this repo. Never create one.
