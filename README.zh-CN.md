...
**当前阶段：Multica 派生应用已在 Windows 本机原生运行；本地登录、诊断概览及 API/Web 进程自动恢复已验证，完整业务与真实执行器验收仍未完成。** main 管理规格、任务及记录；应用代码已发布到 [app-main](https://github.com/899ms/loretide/tree/app-main)，基线 bb3a4b4。应用 PR 必须以 app-main 为目标，本地 app/ 仍是独立 checkout。详见[本机迁移记录](records/2026-09-13-Windows本机开发环境迁移.md)。

文档基线：v0.6.0 · 更新：2026-09-13

## 从这里开始

**开发设计强制规范：**[完整复用 Multica 的组件、布局与设计 token](docs/design/README.md)。所有新页面及开发诊断入口均须遵守。

**开发任务入口：**[实施计划](tasks/plan.md) · [任务清单](tasks/todo.md)。W-01～W-03 已拆为 24 项任务，另有 [2 个架构前置任务](tasks/architecture.md)，后续保留 6 个阶段项和 9 个检查点；另新增 [13项初期完整诊断任务](tasks/diagnostics.md)，诊断出口是LT-009前置。应用工程与原生测试环境已经建立；业务功能仍待实施。环境证据及剩余检查见任务清单和 records，不把运行成功等同全部验收通过。
...
采用模块化单体，明确数据所有权和公开接口，详见 [模块边界与变更回归约束](docs/12-模块边界与变更回归约束.md)。依赖检查、消费者契约和关键流程回归是开发要求，目前尚未实现这些自动保护。

## 初期完整诊断

[完整诊断规格](docs/13-完整开发诊断与操作日志需求.md)包含操作日志、技术日志与错误、跨层追踪、浏览器诊断面板、输入快照、模拟故障复现和回归结果。所有能力初期必做，不采用最小版；后续功能同步接入诊断及测试。目前是开发要求，尚未实现。

## 多 AI 开发协作

先读 [CONTRIBUTING.md](CONTRIBUTING.md)。从 [GitHub Issues](https://github.com/899ms/loretide/issues) 领取 ready 且无负责人的任务，确认唯一执行者后，在独立分支/worktree、测试库和端口中开发，提交 Draft PR。主任务负责审查、合并和关闭 Issue。代码任务必须先提供可获取的应用仓库与基线提交；GitHub 本身不自动启动 AI。
