...
| ARCH-01 acceptance — the module set matches the declared graph | Yes | `scripts/content-boundaries.json` declares the product modules, `diagnostics`, and `idempotency`; the latter is a shared persistence primitive with no product-module dependency. | The graph is checked by `pnpm check:content-boundaries`; product ownership remains unchanged. |
