...
export const overviewSchema=z.object({components:z.array(z.object({name:z.string(),status:z.string(),last_seen:z.string().nullable(),version:z.string(),reason:z.string(),config_state:z.string().optional().default("unknown"),health_state:z.string().optional().default("unknown"),execution_state:z.string().optional().default("not_applicable"),config_observed_at:z.string().nullable().optional().default(null),config_reason:z.string().optional().default("")}).transform(c=>({name:c.name,status:c.status,lastSeen:c.last_seen,version:c.version,reason:c.reason,configState:c.config_state,healthState:c.health_state,executionState:c.execution_state,configObservedAt:c.config_observed_at,configReason:c.config_reason}))),metrics:z.object({sample_count:z.number(),errors:z.number(),p50_ms:z.number(),p95_ms:z.number().nullable(),retries:z.number(),cancelled:z.number(),dropped:z.number(),sink_errors:z.number(),queue_wait_ms:z.number()}).transform(m=>({sampleCount:m.sample_count,errors:m.errors,p50Ms:m.p50_ms,p95Ms:m.p95_ms,retries:m.retries,cancelled:m.cancelled,dropped:m.dropped,sinkErrors:m.sink_errors,queueWaitMs:m.queue_wait_ms})),scenarios:z.array(z.object({id:z.string(),expected_code:z.string(),kind:z.string().catch("fault").default("fault")}).transform(s=>({id:s.id,expectedCode:s.expected_code,kind:s.kind}))),instance:z.string(),build:z.string(),simulation_enabled:z.boolean(),retention_days:z.number(),capacity:z.number()}).transform(o=>({...o,simulationEnabled:o.simulation_enabled,retentionDays:o.retention_days}));
