...
import type { OperatingDiagnosisOption } from "./shared";
...
export function DiagnosisDecisions({ wsId, report, accounts, annotations, topicCards, topicsLoading, topicsFailed }: { wsId: string; report: OpDiagReportVersion; accounts: OperatingDiagnosisOption[]; annotations: ReturnType<typeof useOpDiagAnnotations>["data"]; topicCards: Array<{ id: string; accountId: string | null; label: string }>; topicsLoading: boolean; topicsFailed: boolean }) {
  ...
  const [mode, setMode] = useState<"create" | "link">("create"); const [linkTargetId, setLinkTargetId] = useState("");
  ...
  const matchingCards = topicCards.filter((card) => card.accountId === accountId || card.accountId === null);
  ...
  const saveSuggestion = () => suggestion.mutate({ path: `${path}/suggestions`, body: { body, target_kind: targetKind, target, judgement_ids: [], evidence_refs: refs } });
  const decideEntry = (entry: NonNullable<typeof annotations>["suggestions"][number], decision: "adopt" | "reject") => decide.mutate({ path: `suggestions/${encodeURIComponent(entry.suggestionId)}/decisions`, body: { suggestion_revision: entry.revision, decision, ...(decision === "adopt" && entry.targetKind === "topic_card" ? { mode, ...(mode === "link" ? { link_target_id: linkTargetId } : {}) } : {}), note: "" } });
  return <SettingsSection title={t(($) => $.contentOperatingDiagnosis.blocks.decisions)}><div className="space-y-4">
    ...
    <SettingsCard><div className="space-y-3"><h3 className="text-sm font-medium">{t(($) => $.contentOperatingDiagnosis.suggestion)}</h3>
      ...
      <Select items={accounts.map((entry) => ({ value: entry.id, label: entry.label }))} value={accountId} onValueChange={(value) => { setAccountId(value ?? ""); setLinkTargetId(""); }}><SelectTrigger><SelectValue placeholder={t(($) => $.contentOperatingDiagnosis.account)} /></SelectTrigger><SelectContent>{accounts.map((entry) => <SelectItem value={entry.id} key={entry.id}>{entry.label}</SelectItem>)}</SelectContent></Select>
      ...
      {annotations?.suggestions.map((entry) => {
        ...
        return <article className="space-y-2 border-t pt-3" key={entry.suggestionId}>
          ...
          {!existing && isTopicAdoption && <>
            <Select items={[{ value: "create", label: t(($) => $.contentOperatingDiagnosisDetails.createNewCard) }, { value: "link", label: t(($) => $.contentOperatingDiagnosisDetails.linkExistingCard) }]} value={mode} onValueChange={(value) => setMode((value as typeof mode) ?? "create")}>
        ...
        {!existing && <div className="flex gap-2"><Button size="sm" variant="outline" disabled={decide.isPending || !supportedTarget || (isTopicAdoption && mode === "link" && (!linkTargetId || topicsLoading || topicsFailed || matchingCards.length === 0))} onClick={() => decideEntry(entry, "adopt")}>{t(($) => $.contentOperatingDiagnosis.adopt)}</Button><Button size="sm" variant="outline" disabled={decide.isPending} onClick={() => decideEntry(entry, "reject")}>{t(($) => $.contentOperatingDiagnosis.reject)}</Button></div>}
      </article>; })}
    </div></SettingsCard>
