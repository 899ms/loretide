...
It passed 20 behavior cases for PID reuse rejection, all-root preflight and per-target stop revalidation, read-only status, caller environment restoration, repository-scoped tool detection from an external working directory, caller-directory restoration on successful and failed tool probes, real checkout build-identifier injection including dirty state, fail-closed unavailable Git metadata, healthy repeated start, partial Web failure, path quoting/Web upstream, stale config rejection, PostgreSQL identity mismatch, unattributed Web children, untracked listener rejection, application-role privilege/owner/query-failure rejection, and enforced tool versions. The external-directory cases exercise `Invoke-BootstrapMain` and the real `Get-ToolVersions` path with only the individual command boundary injected; they do not replace the entire version object with the `ToolVersions` hook. The build-identifier case independently reads the checkout's real HEAD and porcelain status, then observes `LORETIDE_BUILD` at the fake process boundary. The Git-failure case proves startup does not invent a build identifier or cross a PostgreSQL, command, or process boundary when metadata is unavailable. The privilege, owner, and query-failure cases prove no migration, build, or process launch occurs after a failed identity check. These tests do not start or stop real services.
...
The launcher now injects the full checkout Git object ID as `LORETIDE_BUILD` and appends `-dirty` when tracked or untracked changes are present. Git metadata is required and is never replaced with `unknown`. The existing manually inspected service was not restarted for this metadata-only correction, so its overview retains the environment from its earlier launch; the corrected value takes effect on the next operator-controlled start.
