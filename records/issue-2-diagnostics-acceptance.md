...
`tests/diagnostics-acceptance/verify.ps1` is the repeatable verifier. Its source-presence checks only confirm that expected wiring remains present; they never prove behavior. Behavior is accepted only from the named Go and Vitest suites, whose JSON reports must show every requested Go test exactly once and both requested Vitest files/two tests passing. Supplying `-DatabaseUrl` is optional and rejected unless it names a credentialed, loopback `localhost:15402` database with the `loretide_diag_acceptance_*` naming convention. Before any database test it requires a PostgreSQL protocol probe proving the requested `current_database`, `current_user`, and non-superuser role; its temporary environment variables and caller working directory are restored in `finally`. It does not echo the URL or password, start Docker, create an environment, inspect model credentials, contact a VPS, or enable a real executor.
