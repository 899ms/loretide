...
- `records/issue-283-t076-mutations.mjs`：四类变异运行器；逐项核对精确源码锚点、命名目标测试的断言级红灯、源文件逐字节还原和还原后绿灯。覆盖 Claim 顺序（T060/T067）、采用预检查提交顺序（T064）、删除工作区错误映射（T071）、复制旧审核请求（T072/T049）。
- `records/issue-283-t076-mutations.md`：隔离容器环境要求、当前测试名、每项预期红灯与交接说明。
- 以上文件仅准备并做脚本语法/差异检查；本机没有执行脚本或数据库测试。T076 最终变异证据由主控在现有隔离数据库容器串行取得。
