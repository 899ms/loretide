...
It 'quotes the workspace Next path and supplies the Web rewrite upstream' {
  ...
  $expectedSha=((& git -C $repoRoot rev-parse --verify HEAD)-join'').Trim();if($LASTEXITCODE){throw 'test setup could not read checkout HEAD'};$expectedStatus=((& git -C $repoRoot status --porcelain=v1 --untracked-files=normal)-join"`n");if($LASTEXITCODE){throw 'test setup could not read checkout status'};$expectedBuild=if([string]::IsNullOrWhiteSpace($expectedStatus)){$expectedSha}else{"$expectedSha-dirty"}
  $script:startCount=0;$hooks['StartProcess']={param($Spec);$script:startCount++;if($script:startCount -eq 1){$identity=New-Identity 4401 $Spec.filePath owned-api;$processes[4401]=$identity;return [pscustomobject]@{Id=4401}};$script:captured=[pscustomobject]@{args=$Spec.arguments;cwd=$Spec.workingDirectory;remote=$env:REMOTE_API_URL;public=$env:NEXT_PUBLIC_API_URL;build=$env:LORETIDE_BUILD};$identity=New-Identity 4402 $Spec.filePath owned-web;$processes[4402]=$identity;[pscustomobject]@{Id=4402}}
...
It 'rejects unavailable checkout build metadata before service boundaries' {
  $output=New-TestRoot build-metadata-failure;New-TestConfig $output|Out-Null;$events=[Collections.Generic.List[object]]::new();$hooks=New-CommonHooks @{} $events @{};$hooks['CheckoutGit']={param($Arguments);throw "synthetic git metadata failure: $($Arguments -join ' ')"};$hooks['StartProcess']={param($Spec);$events.Add("start:$($Spec.name)");throw 'unexpected process launch'}
  Assert-Throws { Invoke-BootstrapMain start 'C:\runtime path' $output 13101 18101 15401 test_database test_role $hooks @{OutputPath=$output} } 'synthetic git metadata failure';Assert-Equal 0 $events.Count 'missing build metadata must fail before PostgreSQL, command, or process boundaries'
}
