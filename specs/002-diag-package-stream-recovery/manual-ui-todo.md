...
`specs/013` 核实时发现，界面实时日志区块的标题写着「不应用上方历史筛选」，而本条要求**筛选跟随**——而**实现一直是筛选跟随**（`useDiagnosticStream` 把筛选项传给流，`stream-state.ts` 有 `filter-change` 转移）。矛盾在文案，不在实现。

**主任务 2026-09-15 裁决：改文案不改实现。** `text058` 四语言已改为如实描述（中文「沿用上方筛选」）。本条现在按正常口径判定即可。
