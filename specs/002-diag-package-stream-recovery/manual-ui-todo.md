...
| V05-3 | 技术日志筛选区 | 设 `severity=error`，保持实时流连接（**口径冲突见下方注**） | 新到事件均为 error；Network 面板中每次续连的查询串都含 `severity=error` 且 `after=<上次游标>` | FR-002 / SC-002 | 待用户验证 |
...
| V05-14 | 诊断页 → 概览 + 技术日志 | **保留期路径**：用 `LORETIDE_DIAG_RETENTION_DAYS=1` 让一批记录过期，再对照概览的 `retention_days` 与技术日志中最早一条的时间 | 早于保留期的记录已不可读取；从早于保留期的游标续读时出现缺口提示（见 V11-5） | D13-V05「保留期」 | 待用户验证 |
| V05-15 | 诊断页 → 概览 + 技术日志 | 跑一次 `shape_sink_failure` 后查看概览，再回到技术日志 | 概览的 `dropped` 指标显示真实数值；界面不把被丢弃的区间显示为完整（配合 V11-2、V11-5） | D13-V05「丢弃提示」 | 待用户验证 |
...
| V08-11 | 诊断页 → 诊断导出 | **授权路径**：从 `handler-tests` 工作区取一个 `run_id`（需在该工作区有成员身份），回到当前工作区触发预览与下载（两条路径都试） | 两条路径都返回 403 / 404 诊断错误对象；界面显示 `next_action`；不生成文件、不泄漏该 run 的任何字段 | D13-V08「跨账号导出拒绝」/ FR-006 | 待用户验证 |
...
| # | 页面 / 位置 | 操作 | 预期 | 对应 | 用户确认 |
|---|---|---|---|---|---|
| V11-1 | 诊断页 → 概览 | 跑一次 `shape_sink_failure`，再看概览指标区（`sink_errors` 与 `dropped` 同时变为非 0） | `sink_errors` 指标可见并显示真实数值；组件状态不显示为全部健康 | spec US3 §3 | 待用户验证 |
| V11-2 | 诊断页 → 概览 | 同 V11-1 的那次 `shape_sink_failure`。**注意来源**：走的是写入失败路径，不是括注的「缓冲区溢出」路径；想看满环路径就连跑 4 次 `shape_deep`（4 × 250，环容量 1000） | `dropped` 指标可见并显示真实数值 | spec US3 §3 | 待用户验证 |
...
| V11-8 | 同上 | 打开实时流后跑一次 `shape_deep`（250 个事件一次写入，specs/012） | 显示「仅显示最近 200 条」；列表条数有界，不无限增长 | FR-008 | 待用户验证 |
...
**V05-14 / V08-11 / V11-8 的造法（主任务 2026-09-15 裁决）**：

- `V05-14` 属**保留期路径**，`V08-11` 属**授权路径**，两者都**不是数据形态**，不由模拟器承担——让模拟器造一条「已过期的记录」或「他人工作区的运行」，等于让它写一条本不该存在的记录。前者用 `LORETIDE_DIAG_RETENTION_DAYS=1`，后者需要 `handler-tests` 工作区的成员身份；没有成员身份就记「未执行 · 缺少授权前置」。
- `V11-8` **已被 `shape_deep` 覆盖**：一次运行写 250 个事件，正好越过「仅显示最近 200 条」的界。

`V11-1` / `V11-2` / `V05-15` **不再依赖 `§9` 的破坏性操作**：`shape_sink_failure` 的失败只作用于该次运行的写入，紧接着跑一次 `normal` 计数就不再增长，实例不需要重建。因此这三条现在可以和概览的其余条目在同一轮里跑完。

## `V05-3` 的口径冲突（`specs/013` 记录，不解决）

界面实时日志区块的标题写着「实时日志 · 最近 200 条（**不应用上方历史筛选**）」（`packages/views/locales/zh-Hans/common.json` 的 `text058`），而本条要求**筛选跟随**。

代码一侧：`useDiagnosticStream` 确实把筛选项传给了流，`stream-state.ts` 有 `filter-change` 转移。**实现与本条一致，与那句标题不一致。**

**建议**：改标题文案，不改实现。裁决前跑本条以「筛选跟随」为准，并在备注写明看到的文案与之矛盾。
