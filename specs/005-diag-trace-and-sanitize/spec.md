...
- Q: 技术事件里用什么形状来表示「这是哪个请求」？ → A: **暂定路由模板 + HTTP 方法**（如 `GET /api/content-diagnostics/events`），主任务尚未确认。这是四个选项里最保守的一个——可证明不含任何用户取值；若后续改为更细的形状，属放宽而非纠错。标注为待确认，见 Assumptions。
...
7. **Given** 一个非诊断的业务路由，**When** 请求经过公共中间件层，**Then** trace 被生成与传播，但**不**写技术事件——记录范围不变。
...
当一条诊断记录必须与业务事务同生共死，但只能在提交之后才能对外投递时，有一个接口承载它：事务回滚则该记录不存在，事务提交则该记录被派发或可见地进入重试，不会在回滚后仍被投递。本功能只交付这个接口（进程内，不落库）；跨进程重启的持久保证是后续任务。
...
- **待主任务确认**：技术事件的请求身份形状暂定为「路由模板 + HTTP 方法」（Clarifications 第 4 条）。这是四个候选里最保守的一个，改为更细的形状属放宽，不会使已写的验收失效。确认前 plan 与 tasks 按此形状推进。
