...
- **FR-002**: 系统 MUST 按每个 span 相对本次运行最早 `occurred_at` 的偏移量横向定位，使等待间隙与并发重叠可见；宽度 MUST 表示该 span 自身耗时。
- **FR-002a**: 定位 MUST 始终基于 `occurred_at` 的实际值，MUST NOT 夹取到父区间内、MUST NOT 改按 `sequence` 重排。当子 span 的开始早于其父 span 时，系统 MUST 在该处标注「时钟偏差」并提示此处不可按时间解读。
...
> 下列五项原为默认假设，已在 2026-09-14 的 Clarifications 中由主任务确认，现为**决定**而非假设，写入对应 FR：前端推导「未运行」（FR-005/FR-010）、200 上限与按错误码保留的折叠策略（FR-004）、时钟偏差下仍按 `occurred_at` 定位并标注（FR-002a）、页内切换不新增路由（FR-008）、不做概览聚合（FR-013）。以下是仍然成立的假设。
