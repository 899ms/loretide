...
- Q: 检查脚本本功能是否加进 `loretide-content.yml`？ → A: **加入**。在**现有 job 内**新增一个 run 步骤调用 `pnpm check:diagnostics-contract`，**触发条件一字不动**。（主任务 2026-09-14 确认。）
- Q: 豁免登记放在哪里？ → A: `scripts/diagnostics-contract.json`（与 `content-boundaries.json` 同目录）。**每条豁免必须带原因与到期日期两个字段；没有到期日期的豁免视为无效。**（主任务 2026-09-14 确认。）
...
- **FR-004**: 交付 MUST 包含一个静态检查脚本，核对每个**已落地**的 content 模块具备诊断接入的最小证据。**本条与 FR-011 的最小证据适用于消费方模块**；提供方模块（`diagnostics` 自身）按接入合同 §5.1 的 P1–P3 判定，见 Assumptions。
...
- **FR-011**: **消费方模块**的「最小证据」MUST 由以下**三条同时满足**构成，缺任一条即判定为无证据（提供方模块不适用本条，见 FR-004 与 Assumptions）：
...
- **FR-012**: 合同 MUST 说明模块的诊断接入写在模块目录之外时如何登记豁免，MUST NOT 只能靠关闭检查绕过。豁免登记在 `scripts/diagnostics-contract.json`。
- **FR-012a**: 每条豁免 MUST 同时带**原因**与**到期日期**。**没有到期日期的豁免 MUST 判为无效**，使检查失败——而不是当作一条永久豁免放行。
- **FR-012b**: 到期日期已过的豁免 MUST 停止生效：该模块重新按 E1/E2/E3 判定，且失败输出 MUST 指明原因是豁免已过期，而不是笼统地报缺证据。
...
- **豁免登记（Exemption）**：模块接入写在别处时的显式记录，含模块名、原因、实际接入位置、**到期日期**。到期日期缺失即无效；已过期即失效。
...
- **SC-002a**: 缺到期日期的豁免使检查退出码非 0；到期日期已过的豁免不再放行该模块，且失败输出指明是**豁免过期**。
...
- `diagnostics` 模块是这套接口的**提供方**，它无法 import 自己，因此 FR-011 的 E1/E2/E3 对它在结构上不可能成立。它按接入合同 §5.1 的**提供方规则 P1–P3** 判定：P1 模块目录内至少一个非测试 `.go` 声明 `package diagnostics`；P2 合同第 2 节第二列点名的公共入口在模块内均有导出声明；P3 模块带有 `_test.go`。该规则比消费方规则更强——它把合同第二列钉成机器核对的对象，改名或删掉其中一个入口检查即变红——且不需要为满足本合同改动 `diagnostics` 的任何生产代码。
- 模块图以 `scripts/content-boundaries.json` 为唯一来源，本功能只读不写。
- 本功能不产出任何手动 UI 验收项——没有页面改动。
- 检查脚本在本功能内加入 `loretide-content.yml` 的现有 job（主任务确认）。现在加的理由是时机——等第一个新模块落地时再加，那个 PR 会同时背上「实现模块」与「引入新红线」两件事，最可能的结果是当场把检查删掉；现在加，它先在 `diagnostics` 上跑绿，新模块落地时自然生效。
- 豁免登记放 `scripts/diagnostics-contract.json`，不碰 `scripts/content-boundaries.json`（ARCH-01/02 的交付物）。**到期日期是必填项**：一条没有期限的豁免会变成永久豁免，而永久豁免与删掉检查的效果相同。
