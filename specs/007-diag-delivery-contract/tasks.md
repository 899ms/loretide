...
- US1：T005 → T006 / T007 / T008（同一文件，顺序写）→ T009（核对，依赖前四条写完）
- US2：T010 → T011 → T012 → T013；T014 依赖 T003、T013；T015 依赖 T013；T016、T017 依赖 T015
- US3：T018 依赖 T005（同一文件）；T019 依赖 T016、T018
- Polish：T020 依赖 US2；**T021 → T022**；T023 依赖全部；T024 最后

## Parallel Example

```text
并行组 A（Foundational）：T003 豁免配置 | T004 只读复核
并行组 B（跨故事）：US1 的 T005～T008（文档） | US2 的 T010/T011（脚本）
```
