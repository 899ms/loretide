...
| 为什么两者都要 | **实施期修正**：两者买的不是同一件事。**租约保证正确性**——它写在行上，活过认领方的连接，第二个排水器在第一个提交后重读该行时 `claimed_until` 已被推到未来，于是跳过。**`SKIP LOCKED` 只买不阻塞**——没有它，第二个排水器会阻塞等第一个提交（之后同样跳过，结果仍然正确），排水器因此串行排队而不是分摊同一批 |
