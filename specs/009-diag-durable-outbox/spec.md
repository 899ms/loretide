...
### Session 2026-09-14（待主任务裁决，已按推荐值暂定，不阻塞）

- **Q1**（调用点）：本特性是否同时接一个生产调用点，好让「与业务写入同事务」有真实事务可验证？→ **暂定 A**：新增一个 `Store` 上的入口，在**同一个事务**里完成一次业务写入与一次 `Register`，提交后结算；`Store.Audit` 的行为一字不动。见 FR-005。
- **Q2**（排水器的归属与触发）：谁运行排水器、以什么节奏？→ **暂定 A**：两段式——提交后**就地**排水本事务的记录（低延迟），外加进程内**周期扫描**认领遗留与租约过期的记录（这才是重启恢复的来源）。不新增独立后台服务。见 FR-008、FR-009。
- **Q3**（`tx any` 拿不到真事务时）：落库实现在类型断言失败时怎么办？→ **暂定 A**：**失败关闭**——`Register` 返回错误，调用方的事务随之回滚。「以为写进了 outbox 其实没写」比直接失败危险得多。见 FR-004。
...
- **FR-004**: 落库实现在收到的事务句柄不是一个可用的数据库事务时 MUST 失败关闭：返回错误、不暂存、不静默接受。（Q3 暂定 A）

**同事务写入**

- **FR-005**: 系统 MUST 提供一条可被端到端验证的路径，在**同一个事务**内完成一次业务写入与一次记录登记，并在该事务提交后结算。（Q1 暂定 A：新增入口承载，`Store.Audit` 行为不变）
...
- **FR-009**: 系统 MUST 另有一条周期性的扫描路径，认领并派发**任何**尚未派发的记录，不依赖登记它的那个进程仍然存活。（Q2 暂定 A）
- **FR-010**: 进程重启后，已提交但未派发的记录 MUST 仍被派发。
- **FR-011**: 任何记录 MUST 最多被实际派发一次，无论重启发生在哪一步、也无论同时有几个排水器在跑。
...
- **FR-018**: 记录 MUST 有重试上限与失败后的终止状态，MUST NOT 无限重试；进入终止状态 MUST 可被观察到。
...
- **SC-001**: 一条已提交未派发的记录，在丢弃全部进程内状态后由新实例排水，被派发且**恰好一次**。
