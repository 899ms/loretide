...
### analyze 自行整改的两条（2026-09-15）

| ID | 严重度 | 问题 | 整改 |
|---|---|---|---|
| **C1** | MEDIUM | FR-019（「必须改生产代码才能测就停下来报告」）**没有任何任务承载**。它是本特性唯一一条「遇到什么情况要停」的规则，只写在测试政策里而没进任务清单，等于指望执行者记得 | 并入 T026，并要求交付记录写明**是哪一条、报告内容与裁决**，禁止自行改动后事后说明 |
| **C2** | MEDIUM | SC-009 只说「§4.1 与第 2 节一致」，**没有给出可比对的数字**。「一致」在两边同时数错时也成立 | data-model 新增「§4.1 计数的预期落点」一节，把 113/24/8 → **122/15/8** 的算术写出来并说明剩下的 15 行是谁；SC-009 写进该数字 |

另做了一次**规格声明的实测核对**（不是整改，是验证）：FR-011b 要求「静态检查对当前仓库退出 0」。四个清单文件按词边界匹配的命中数**实测均为 0**（`index.tsx` 朴素匹配 4、词边界 0，其余三个两种匹配都是 0）。这条正例声明因此是核实过的，不是假设。

### 下一步

`/speckit-implement`（限本特性的任务号与文件范围）。三处 Q 已裁决，FR 22 条、SC 12 条、任务 30 条，coverage 无缺口。
