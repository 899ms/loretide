...
- **Q1**（DIAG-03 级别配置）：按级别过滤写入的配置路径在生产代码里不存在，不改生产代码就关不掉这一行。怎么处理？→ **暂定 A**：把该行重新界定为它**今天真正交付的东西**——`Severity` 受限于四值枚举、`limits.go` 两项可配置且拒绝非法值——两者均已有测试，因此**只改对照表引用与措辞**；同时把「按级别过滤写入」作为**缺失功能**单列为后续任务，不在本特性内实现。见 FR-003、FR-014。
- **Q2**（两条未点名的非界面行）：DIAG-02 验证嵌套字段、DIAG-10 不复制媒体，纳不纳入？→ **暂定 A**：**都不纳入**。DIAG-10 已被 spec 008 认领，重复会撞车；DIAG-02 嵌套字段在 `Event` 为扁平结构时造不出真实负例，宜与 008 的快照字段清单一并处理。两者在本特性的交付记录里**显式列为不在范围及原因**。见 FR-015。
- **Q3**（DIAG-12 不自动上传的核对形式）：→ **暂定 A**：写成 **`node:test` 静态检查**，与 `scripts/check-diagnostics-contract.mjs` 同形：扫描导出相关的前端文件，断言其中不出现 `fetch` / `XMLHttpRequest` / `sendBeacon` / `new WebSocket`。理由是这条性质上是「代码里没有某样东西」，静态检查能表达，运行时测试反而要靠「没观察到」来证明。手动项 V08-8 保留不动。见 FR-011、FR-012。
...
- **FR-003**: 写测试也关不掉的行，交付 MUST 在对照表写明缺的是**功能**而非测试，并 MUST 指向单列的后续任务（Q1 暂定 A）。
...
- **FR-011**: MUST 有一条核对证明导出/保存路径**不发起任何外发请求**（Q3 暂定 A：`node:test` 静态检查）。
- **FR-012**: FR-011 的核对 MUST 在被检查的文件清单过时时**失败而不是静默通过**（例如断言被检查的文件确实存在）。
- **FR-013**: 每条新增测试 MUST 在对应实现规则被删除或反转时变红（变异验证）。

### 范围与边界（US3）

- **FR-014**: 「按级别过滤写入」MUST 作为**后续任务**单列，MUST NOT 在本特性内实现——本特性 MUST NOT 改动任何生产代码。
- **FR-015**: DIAG-02「验证嵌套字段」与 DIAG-10「不复制媒体」MUST 显式列为不在本特性范围，并写明原因（后者已被 `specs/008` 认领）（Q2 暂定 A）。
...
- **SC-009**: 对照表 §4.1 的三列计数与第 2 节逐行统计一致。
...
- 三处 Q1/Q2/Q3 已按推荐值暂定并写入 FR，主任务裁决若不同，改动范围各自局限在一条 FR 与其对应任务。
