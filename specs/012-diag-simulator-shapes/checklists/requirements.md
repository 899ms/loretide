...
**已关闭（2026-09-15 主任务裁决）**：4 条是 `008-J-4`、`002-V05-14`（保留期路径）、`002-V08-11`（授权路径）、`002-V11-08`（**已被 `shape_deep` 覆盖**）。抬出这个差额是对的——其中一条本来就已经做到了，另三条则会一直挂在「模拟器造不出」的名下，而它们根本不该由模拟器承担。

### 三处 Q 按推荐值暂定，不阻塞

| 项 | 问题 | 推荐值 |
|---|---|---|
| Q1 | 19 与 15 的差额是哪 4 条 | 覆盖点名的 15 条，差异写进交付记录。**2026-09-15 已裁决并关闭**，见上 |
