...
**覆盖核对**：9 个形态覆盖 13 条；`006-L-5` 与 `008-O-2` 显式移出（FR-020 / FR-021）；13 + 2 = **15**，等于任务描述 (a)～(h) 点名的条数。

**Q1 已裁决（2026-09-15）**：差的 4 条是 `008-J-4`、`002-V05-14`、`002-V08-11`、`002-V11-08`。`002-V11-08`（> 200 条实时事件）**已被 `shape_deep` 覆盖**——一次运行写 250 个事件，正好越过「仅显示最近 200 条」的界；另三条分属保留期与授权路径，不是数据形态。最终：**已覆盖 14 条、移出／转路径 5 条、仍不可达 0 条**，合计 19。
