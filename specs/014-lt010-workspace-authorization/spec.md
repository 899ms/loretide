...
三项涉及结构与产品语义，**已提交主任务裁决，尚未回答**。下列 A 为本会话采用的**暂定值**，正文按暂定值写就，使 plan / tasks / analyze 不被阻塞。

- **Q1 助手放哪？** → **A（暂定）**：新建 `server/internal/content/workspace-core/`。**影响** plan 的 Source Code 清单、接入合同判定。
- **Q2 拒绝响应统一成什么形状？** → **A（暂定）**：助手返回**类型化判定**，不自己写响应；诊断沿用它现有的 404/403 两种状态码（逐字节不变），助手另提供**新模块用的规范映射**——无权与不存在**同为 404**。**影响** FR-004 ～ FR-007、契约。
- **Q3 是否允许缓存成员关系？** → **A（暂定）**：不缓存，每次判定都查。**影响** FR-009、并发归属变化用例。
...
- **FR-013**: 若新建 `server/internal/content/<module>/` 目录，MUST 满足接入合同 E1/E2/E3，且 `pnpm check:diagnostics-contract` 退出 0。
