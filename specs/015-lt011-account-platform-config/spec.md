...
三项已提交主任务裁决，**尚未回答**；下列 A 为暂定值，正文按暂定值写就，不阻塞后续阶段。详见文末。

- **Q1 落哪个登记模块？** → **A（暂定）**：`ip-profile`。
- **Q2 `workspace_id` / `id` 列用 text 还是 uuid？** → **A（暂定）**：`text`，与同层三张 content 表一致。
- **Q3 平台枚举在哪校验？** → **A（暂定）**：Go 侧受控枚举为准（产出 400 诊断错误对象）+ 库加 `CHECK` 作最后防线。
