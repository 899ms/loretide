...
三项**均已裁决为 A**，与推荐值一致，正文无需回改。裁决另附三条要求，已并入下方条款：

- **Q1** → **A（已裁决）**：独立不可变版本表，**只插不改不删**。
- **Q2** → **A（已裁决）**：不存指针；当前版本 = 该账号 `revision` 最大行。并发两次更新靠 `(account_id, revision)` 唯一约束互斥，**冲突方重试或返回 409 诊断错误对象——规格必须写明选哪种并有用例**。
- **Q3** → **A（已裁决）**：`revision_id text` 主键 + `(account_id, revision)` 唯一；**唯一约束用独立迁移文件的 `CREATE UNIQUE INDEX CONCURRENTLY`，不写在建表语句里**。
- **后续运行读取「确定快照」时以 `revision_id` 为准**（见 Key Entities）。

### 并发冲突：选「有界重试，仍冲突才 409」

裁决把选择留给规格，这里定下来并说明理由。

**选：冲突后有界重试（最多 3 次），仍然冲突才返回 409 诊断错误对象。**

不选「直接 409」，是因为它与 US5 的验收冲突：两个人几乎同时确认，**两次确认都真实发生过**，历史应当两条都留下（版本 1 与 2）。直接 409 会让其中一次确认**消失**，而创作者那边看到的是一个失败——他不知道自己该不该重做。

不选「无限重试」，是因为持续争用下它会把一次写入拖成不确定时长的循环。3 次之后如实回 409，让调用方决定。
...
- **FR-009**: 并发写入 MUST NOT 产生重复版本号，也 MUST NOT 静默丢写。冲突时 MUST **有界重试（≤3 次）**；仍冲突 MUST 返回 **409 诊断错误对象**，MUST NOT 无限重试。
- **FR-009a**: `(account_id, revision)` 的唯一约束 MUST 由**独立迁移文件**的 `CREATE UNIQUE INDEX CONCURRENTLY` 建立，MUST NOT 写在建表语句里（Q3 裁决）。
...
- **运行引用**：运行钉住一个 **`revision_id`**——后续运行读取「确定快照」时**以 `revision_id` 为准**，不以版本号、不以账号加时间。版本号 `revision` 只用于人读与排序（「这是第几版」）；跨表引用一律用 `revision_id`。**本卡不实现运行**，只保证版本可被稳定引用。
...
- **SC-005**: 并发两次设置产生版本号 1 与 2，无重复、无丢写；重试次数耗尽时返回 409 而非死循环。
