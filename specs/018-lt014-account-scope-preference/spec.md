...
**一个必须说清的限制**：今天**诊断运行还无法关联到一个内容账号**。`content_diagnostics.go:36` 的 `diagnosticScope` 把 `scope.Accounts` 置为空切片，注释写着「Real account permissions are connected when the account domain ships」，于是带 `account_id` 的运行一律 403。

这意味着「运行不回写账号偏好」**今天成立的原因是运行根本不认识账号**，不是因为有什么东西拦着它。本卡的断言因此是一条**前向守卫**：它现在必然通过，价值在于将来账号与运行接上时，任何「顺手把偏好写回去」的改动会让它变红。变异验证里我用「运行结束后给工作区内所有账号写偏好」来证明这条守卫**不是空的**——那次变异确实让它红了。**不把它说成「已经证明今天受保护」。**
...
三个问题已由主任务**裁决为 A**：settings 的 `loretide.scope` 键（零迁移，照 `#63` 时区手法）；专用 `PUT /api/content-accounts/{id}/scope` 服务端读-改-写合并；账号页不加控件。

**裁决追加的一条**：`PATCH /{id}` **整体替换 settings** 这一行为在 PR 正文单列为「已知行为」，并加一条用例**钉住**「`PATCH` 只传 `loretide.scope` 会抹掉其他键」——**不改它**，留给以后需要时再定。

因此本卡多一条要求：

- **FR-017**：MUST 有一条用例断言 `PATCH /{id}` 只携带 `loretide.scope` 时其他 `settings` 键**被抹掉**。这条用例记录的是**当前行为**，不是期望行为；它存在是为了让这个行为在被改变时必须是一个决定，而不是一次意外。
