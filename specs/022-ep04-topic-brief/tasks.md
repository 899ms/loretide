...
- [x] T035 `packages/views/content/topic-planning/`：**只挂既有组件**（SOP 阶段 UI 规则），不新增控件、不调样式。 — **FR-022**
- [x] T036 七项字段的表单、四个动作的按钮、暂缓 / 放弃的原因与备注输入。 — **FR-001 ～ FR-004**
- [x] T037 简报版本的查看与修改；修改产生新版本，旧版本可读。 — **FR-007 ～ FR-009**
- [x] T038 文案四语言齐全，`locales/parity.test.ts` 通过。 — **FR-022**
- [x] T039 `manual-ui-todo.md` 写齐：七项可填、四个动作、原因与备注、改简报产生新版、旧版可读、跨品牌隔离、页面与既有设置页视觉一致。 — **FR-021**
- [x] T040 `pnpm typecheck --force`；**新增 UI 单测数为 0**（SC-013）。 — **FR-021、SC-013**
- [x] T041 `tasks.md` 回勾；界面各条记「未执行」（只能由主任务在浏览器给出）。 — **FR-023**
