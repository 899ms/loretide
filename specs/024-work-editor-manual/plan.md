...
> **三条 clarify 已裁决（主控 2026-09-21），并给出了 SOP §7.1 原文。** Q1=A（不自动存版、要 `workspace_id` 打头的索引）、Q2=A、Q3 严格按原文。本计划已按裁决改写；裁决改变了什么，见 [spec.md → 裁决记录](./spec.md#裁决记录主控-2026-09-21)。
...
**Storage**: PostgreSQL。**三张新表 + 七个 CONCURRENTLY 索引**，十个迁移文件（494–503），每个单条语句；建表迁移不含 `PRIMARY KEY` / `UNIQUE`；**每张表都有一个以 `workspace_id` 打头的索引**，否则工作区删除会在持锁期间扫全表。
...
| V. 无外键、无级联 | **需注意** | 三张新表 + 七个索引，是这张卡最容易违规的地方。R1–R6 全生效；见下 |
...
| R3 | 每个索引 `CREATE INDEX CONCURRENTLY` | 495 / 496 / 498 / 499 / 501 / 502 / 503 |
| R4 | 建并发索引的文件只能有这一条语句 | 七个索引各自成文件 |
| **R5** | 建表迁移**不得**有 `PRIMARY KEY` / `UNIQUE` | 494 / 497 / 500 只有列与 `CHECK`；三个 id 的唯一性走 495 / 498 / 501 |
| **R6** | 建索引的 up 迁移逐条登记，索引名逐字相同 | **七条**登记；**494 / 497 / 500 不登记**（登记一个不存在的索引同样是静默 no-op，R6 的反向断言会红） |
...
**编辑副本状态 `working` / `saved` 是存下来的，不是每次读时重算**（重算要取最新版本全文，列文档时就是 N 份）。代价是两个真相里可能有一个会错，所以**改动它的三处写路径各要一条用例，断言存下来的与重算的一致**（FR-007b）。
...
server/
  migrations/
    494_content_work.{up,down}.sql                     # 建表，无 PK/UNIQUE（R5），无 status 列
    495_content_work_id_unique_idx.{up,down}.sql
    496_content_work_card_idx.{up,down}.sql            # (workspace_id, topic_card_id, created_at DESC)
    497_content_artifact.{up,down}.sql                 # 含 draft_body / draft_status 可变列
    498_content_artifact_id_unique_idx.{up,down}.sql
    499_content_artifact_work_idx.{up,down}.sql
    500_content_artifact_version.{up,down}.sql         # source 与 action 两列
    501_content_artifact_version_id_unique_idx.{up,down}.sql
    502_content_artifact_version_unique_idx.{up,down}.sql   # (artifact_id, revision)
    503_content_artifact_version_workspace_idx.{up,down}.sql # 删除链不走全表扫
  pkg/db/queries/
...
| `GET` / `PATCH` | `/api/content-works/{id}` | 读 / 改标题（**本卡没有状态列**） |
...
| `POST` | `/api/content-works/{id}/artifacts/{artifactId}/versions/{versionId}/adopt` | 采用为基线 → **产生新版本**（`source`/`action` 都是 `adopted`） |
...
`server/cmd/server/router.go` 是上游文件。#71/#73 的教训是「handler 写完了没挂路由，什么都到不了它」。同一个 `upstream:` 提交里还有 `server/cmd/migrate/main.go` 的**七条索引登记**——#122 刚证明过，不登记的并发索引，被中断的建索引会在重试时被记成成功，索引永久不可用且运行时没有任何提示，而 **R6 会让它在 CI 上变红**。PR 正文单列「上游改动」一节（工作流第 13 步）。

## 两个 PR 的分界

| PR | 内容 | 验收 |
|---|---|---|
| **PR 1 存储与接口** | 十个迁移、删除链与清单、`contract.go` / `store.go`、八个 handler、路由与索引登记（同一 `upstream:` 提交）、core 契约与 hooks、Go 用例 | 建作品/文档/存版本能跑通；自动保存不产生版本；恢复产生新版本且旧版逐字节不变；版本只插不改不删（守卫）；并发不撞号；越权与不存在同形；模块里没有产生 `generated` 的路径；编辑副本状态与重算一致；R1–R6 与删除清单全绿；第 12 步用例 |
| **PR 2 页面** | 编辑器页、版本历史侧栏（**来源与动作两列都显示**）、三个禁用的 AI 入口、四语言、`manual-ui-todo.md` | 自动保存可见；`working` / `saved` 两个状态都看得出来；历史可读、可恢复、可采用；AI 入口禁用且有原因；**无 UI 单测** |
...
| 采用产生新版本而不是一个指针 | 指针可变，「采用过哪几版」这段历史就没了；而布尔标记要把旧标记改回 false，那是给 append-only 表开 UPDATE，与 FR-008 直接冲突 |
| 来源与动作是两列而不是一列 | §7.1 原文就是「来源与动作记录」。压成一列会逼读者去看另一列才知道发生了什么——而「恢复」的内容仍是人写的，来源理应还是 `edited` |
| 编辑副本状态存而不算 | 重算要取最新版本全文；列文档端点就是 N 份。代价用 FR-007b 的一致性用例兜住 |
| 编辑副本是列而不是表 | 文档行本来就可变（标题、序号会改），可变列不破坏任何守卫；单独一张表只是多一张表、一组索引和一条删除链 |

## 本卡验不了的两件事（写在这里，不留到实施时才发现）

1. **「已审核、已交接、已发布版本始终保留」（§7.1 原文）只有结构保证。** 本卡连那几个状态值都不落地，所以能验的是「没有任何删除版本的路径」，不是「一条已发布的版本真的挺过了某次清理」。后者要等 review-delivery 与交付卡。
