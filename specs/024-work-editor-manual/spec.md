...
**Status**: Draft（三条 clarify 已裁决 2026-09-21，含 SOP §7.1 原文，见文末「裁决记录」）

**Input**: Issue #137。SOP §6.2「没有模型时仍可人工写作、编辑、审核和记录反馈」；§7 作品容器 / 文档 / 版本历史；§7.1「文档编辑」行。

**§7.1「文档编辑」行原文**（主控 2026-09-21 提供）：
...
- **编辑副本与最新版本不一致**：这是常态（写了但还没存版本），正是 `working` 这个状态的含义，**不是**错误。反过来 `saved` 表示两者逐字相同。
...
- **FR-004**：作品在本卡 MUST NOT 有状态列。§7.1「文档编辑」行给的状态只有 `working` / `saved`，那是**编辑副本**的状态；`drafting` / `in_review` / … / `published` 属于 §7.1 其它行（审核与交付的对象），不在本卡。加一列比改一列容易，所以留给那些卡去加。
...
- **FR-007a**：编辑副本 MUST 有一个状态，取值**只有** `working` 与 `saved`（§7.1 原文）。`saved` 表示编辑副本与该文档最新版本**逐字相同**；`working` 表示有尚未存为版本的改动。
- **FR-007b**：编辑副本状态被**存储**而不是每次读时重算（长正文逐字比较太贵），因此 MUST 有一条用例，在每一处会改动它的写路径上，断言**存下来的状态与重算的结果一致**。两个真相里有一个会错，用例是唯一能让它出声的东西。

**版本历史**

- **FR-008**：版本 MUST 只插不改不删：没有更新端点、没有删除端点、模块源码里没有 `UPDATE`/`DELETE`（工作区删除链那一条除外），并有守卫用例。
- **FR-009**：自动保存 MUST NOT 产生版本。版本的产生时机见 **Q1**。
- **FR-010**：版本 MUST 记录**来源**，受控集**恰好**是 `generated` / `edited` / `adopted`（§7.1 原文，不多不少）。`generated` MUST **留位但本阶段产生不出来**，并有一条负例钉住「模块里没有产生它的路径」。
- **FR-010a**：版本 MUST 另记一列**动作**，与来源**分开两列**。「恢复」不是第四种来源，是一次动作。两列都 MUST 能读回，页面 MUST 两列都显示。
- **FR-011**：恢复一条旧版本 MUST 产生一条**新版本**，MUST NOT 改写或删除任何既有版本。新版本的 `source` MUST 是 `edited`（内容是人写的，只是从旧版取来），`action` MUST 是 `restored`，并 MUST 记下 `restored_from = <被恢复版本的 id>`。
...
- **FR-016**：系统 MUST 能把某一版采用为「下一步工作基线」（§7.1 原文）。采用 MUST 产生一条**新版本**，`source` = `adopted`、`action` = `adopted`、`adopted_from = <被采用版本的 id>`，内容与被采用版**逐字相同**。MUST NOT 用一个可变指针代替——指针可变，「采用过哪几版」这段历史就没了。
...
- **FR-021a**：每张新表 MUST 有一个以 `workspace_id` 打头的索引。工作区删除按 `workspace_id` 删，而删除要在一个事务里完成；没有这样的索引就是在持锁期间扫全表。
...
- **作品（Work）**：一次产出的容器。挂在一张选题卡上，可引用一个开始快照（可空）。有标题，**本卡没有状态列**（FR-004）。**可变**（标题会改）。
- **文档（Artifact）**：作品下的一篇。有受控类型、标题、显式序号，以及**一份可变的编辑副本**与它的状态（`working` / `saved`）。
- **版本（ArtifactVersion）**：一篇文档的一次快照。**只插不改不删**。有稳定的 `version_id`、每文档自增的 `revision` 计数器、**来源**（`generated` / `edited` / `adopted`）、**动作**（与来源分开的第二列），以及可选的 `restored_from` / `adopted_from` 指针。
...
- **SC-002**：连续自动保存 N 次后，版本数仍为 0 且编辑副本状态为 `working`；显式存版本后版本数为 1 且状态为 `saved`。**自动保存不产生版本**与**状态随之改变**是两条用例，不合并。
- **SC-003**：恢复第 1 版后版本数 +1；新版本 `source = edited`、`action = restored`、`restored_from` 指向第 1 版；第 1 版的内容、来源、动作、时间戳**逐字节不变**。
- **SC-003a**：采用第 2 版后版本数 +1；新版本 `source = adopted`、`action = adopted`、`adopted_from` 指向第 2 版，内容与第 2 版逐字相同；第 2 版**逐字节不变**。
...
- **SC-006**：模块里**没有任何**产生 `source = generated` 的路径，有一条用例在出现时变红。扫的是路径不是当前数据——后者在空库上恒绿。
- **SC-006a**：编辑副本状态的每一处写路径上，**存下来的状态与重算的结果一致**（FR-007b）。
- **SC-007**：三个 AI 入口在界面上**永远显示原因**，且不存在任何伪造的候选或润色结果。
- **SC-008**：迁移规则 **R1–R6 逐条**全绿；R5 专门确认**三个**建表迁移都不含 `PRIMARY KEY` / `UNIQUE`；R6 确认**七条**索引登记名逐字相同且三个建表迁移**未**被登记。
- **SC-008a**：三张新表**各有**一个以 `workspace_id` 打头的索引（FR-021a）。
...
4. **本卡不落地审核与交接，也不落地它们的状态值**。§7.1「文档编辑」行给的状态**只有** `working` / `saved`；`已审核 / 已交接 / 已发布`来自 §7.1 的**其它行**，属 review-delivery（EP-06/EP-07）与交付卡。本卡对「已审核、已交接、已发布版本始终保留」（§7.1 原文）只给**结构保证**——**没有任何删除版本的路径**——与 EP-04a 对 §5.3 的处理口径一致。
...
三条 clarify 与两个附带问题已裁决，**并给出了 SOP §7.1「文档编辑」行的原文**（见本文件开头「Input」）。规格、计划、任务与合同均已按裁决回写。原选项表保留在 PR #139 的正文里，此处只记结论与它改变了什么。

### Q1 = **A**，附带 1 = **不要**，附带 2 = **要第六个索引**

- 编辑副本是 `content_artifact` 行上的可变列；版本在独立 append-only 表里，由**显式**「存为版本」产生。
- **不要**「离开页面自动存一版」的兜底——那会让历史里混进人没有决定过的条目。
- **要**以 `workspace_id` 打头的版本索引，单文件 CONCURRENTLY + 登记清理。

**顺带补上一处我原来漏的**：`content_work` 的唯一索引是 `(work_id)`，同样不以 `workspace_id` 打头，所以**它也需要**一个。而「列出一张卡下的作品」本来就需要 `(workspace_id, topic_card_id, created_at DESC)`——同一个索引服务两件事。因此索引由五个变**七个**，迁移由八个变**十个**（494–503）。这不是扩大范围，是补上一个本来就需要的读路径索引。

### Q2 = **A**

只存 `topic_card_id` / `snapshot_id` 字符串，**不 import** `topic-planning`，存在性由 adapter 校验；`scripts/content-boundaries.json` **不动**。选题卡今天没有删除路径，「卡被删怎么办」进 Out of Scope，已接受。
...
| 处 | 我暂定的 | 裁决（按原文） |
|---|---|---|
| 状态集 | `drafting` / `in_review` / `revising` / `approved` / `handed_off` / `published` | **只有 `working` / `saved`**，而且它们是**编辑副本**的状态，不是作品的。`drafting…published` 属 §7.1 其它行，**不进本卡** |
| 作品状态列 | 有 | **本卡没有**（FR-004）。加一列比改一列容易，留给审核与交付那些卡 |
| 版本来源 | `edited` / `restored` / `adopted` / `generated` 四个 | **恰好三个**：`generated` / `edited` / `adopted` |
| 「恢复」 | 第四种来源 `restored` | **不是来源，是动作**：`source = edited`、`action = restored`、`restored_from` |
| 「采用」 | 来源 `adopted` | 来源**与动作都**是 `adopted`：`source = adopted`、`action = adopted`、`adopted_from`。内容逐字复制（即推荐的 A） |
| 列数 | 一列 `source` | **两列**：`source` 与 `action`，历史侧栏两列都显示 |

**我补了一个裁决没有点名的值**：一次普通的「存为版本」，`source = edited`，那么 `action` 是什么？裁决只点名了 `restored` 与 `adopted` 两个动作。我取 **`saved`**，与编辑副本状态的用词一致（存下来了）。**若应当是空串，请说一声——改动是一个枚举值。**

### 其余全部接受

版本号两步写法（Issue #109）、守卫同时断言「有 INSERT」、`generated` 负例扫路径不扫数据、不引入 diff 库、AI 入口禁用不隐藏、两件只做结构保证——原样保留。
