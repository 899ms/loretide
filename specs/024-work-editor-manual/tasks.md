...
**三条 clarify 未裁决**：本清单按推荐值 **Q1=A / Q2=A / Q3=A**（状态集合用暂定值）写。**T001 是「拿到裁决」**：

- **Q1 选 B** → T003 起的建表任务由三张表变四张，索引由五个变七个；
- **Q2 选 B** → 多一条 `scripts/content-boundaries.json` 的依赖方向改动（跨模块决定）；
- **Q3** 的状态集合若与暂定值不同 → 改一次 `CHECK` 迁移与一个 Go 枚举，**不影响表结构**；`adopted` 若选 B（指针列）→ 少一个枚举值、多一个可变列、`adopt` 端点由 POST 改 PATCH。
- **Q1 附带问题 2**（第六个索引）→ 决定 T007 是否存在。
...
- [ ] T001 **拿到三条 clarify（含两个附带问题）的裁决**并回写 `spec.md` 的「待裁决」一节与 `contracts/` 的状态行。**Q3 需要 SOP §7.1「文档编辑」行的原文**
...
## Phase 2: 存储（八个迁移，各单条语句）

- [ ] T003 `494_content_work.{up,down}.sql`：建表。**无 `REFERENCES`/`CASCADE`（R1/R2）、无 `PRIMARY KEY`/`UNIQUE`（R5）、不建索引（R4）**；`status` 带 `CHECK`；`snapshot_id` 可为 `''` 并在注释里写明它是真实状态。`down` 注明已写内容会丢
- [ ] T004 [P] `495_content_work_id_unique_idx`：`(work_id)`，单文件单语句
- [ ] T005 `496_content_artifact.{up,down}.sql`：建表，含 `kind` 的 `CHECK` 与**可变的 `draft_body` / `draft_saved_at`**（Q1=A）
- [ ] T006 [P] `497_content_artifact_id_unique_idx` `(artifact_id)`；`498_content_artifact_work_idx` `(workspace_id, work_id, position)`
- [ ] T007 **视 Q1 附带问题 2 的裁决**：`content_artifact_version` 的工作区删除索引 `(workspace_id, artifact_id, revision DESC)`。不加则删除链在版本表上走全表扫，**在 PR 正文里如实写明**
- [ ] T008 `499_content_artifact_version.{up,down}.sql`：建表，`source` 带 `CHECK`（含**留位的 `generated`**）
- [ ] T009 [P] `500_content_artifact_version_id_unique_idx` `(version_id)`；`501_content_artifact_version_unique_idx` `(artifact_id, revision)`——**后者既是唯一性也是并发互斥**
...
- [ ] T011 迁移规则核对，**四条都要跑到**：`go test ./internal/migrations -run 'TestContentMigrationConstraints|TestContentConcurrentIndexRegistration'`、`go test ./cmd/migrate -run 'TestEveryConcurrentUpBuildHasCleanup|TestConcurrentIndexCleanupsMatchTheirMigrations'`。**R5 专门确认三个建表迁移都不含 PK/UNIQUE**

---

## Phase 3: 模块（先写测试）

- [ ] T012 [P] **先写** `contract_test.go` 的受控集矩阵：`kind` / `status` / `source` 越界各一条 → `ErrInvalid`；长度上限**按 rune 计数**（中文用例）；空内容合法。确认失败
...
- [ ] T015 新建 `contract.go`：`Work` / `Artifact` / `ArtifactVersion`、三个受控集、`ErrInvalid` / `ErrNotFound` / `ErrStorage`
- [ ] T016 新建 `store.go` 的写路径：**栅栏是事务第一句**（`LockForContentDiagnosticWrite`，#104）；建作品 / 建文档 / PATCH 文档（**只动编辑副本与标题序号，不产生版本**）
- [ ] T017 **先写** 版本号并发用例：N 个并发「存为一版」→ **N 条版本号互不相同**，0 条失败到调用方（冲突方重试后成功）。确认失败
- [ ] T018 `store.go` 的存版本路径：**照抄 022 `AppendBrief` 的两步写法**——先锁文档行，**再在第二条语句里数号**。理由（Issue #109）写进注释：READ COMMITTED 下等锁不刷新语句快照，在锁定语句里数号会选到已被用掉的号
- [ ] T019 **先写** 恢复与采用：各产生**新版本**；被引用的旧版**逐字节不变**；恢复**同时**把编辑副本改成那份内容。确认失败
...
- [ ] T023 **先写** 自动保存不产生版本：连续 PATCH N 次 → 版本数仍为 0；再 POST 一次 → 为 1。确认失败
...
- [ ] T026 **两个上游文件，同一个 `upstream:` 提交**，PR 正文单列「上游改动」一节（第 13 步）：
      ...
      (b) `server/cmd/migrate/main.go` 的 `concurrentIndexCleanups` 加 495/497/498/500/501（含 T007 则六条），索引名**逐字**对上迁移文件。**建表迁移不登记。**
...
- [ ] T030 「有未存为版本的改动」的提示；版本历史侧栏（来源那一列显示 `edited` / `restored` / `adopted` 的区别）
...
- [ ] T035 变异验证五处，每处确认对应用例变红、**改完即还原**。变异必须**可编译**：
      ...
      (M5) 从 `concurrentIndexCleanups` 删掉 501 的登记 → T011 的 R6 变红
- [ ] T036 核对改动文件全部落在 plan.md 清单内；清单外的在 PR 正文单列
- [ ] T037 PR 正文：迁移说明（三张表 + 五/六个 CONCURRENTLY 索引、无外键、R5 为何不用 `PRIMARY KEY`、R6 逐条登记）、**上游改动一节**、UI 影响（复用了哪些既有组件）、「SOP 对应」逐句写明现在可操作到什么程度。**两件本卡验不了的事如实写**：「已审核/交接/发布版本永不删除」只有结构保证；「EP-08 接上不必改调用方」今天无法证明
...
```text
T001 裁决 → T002 基线
  └─ 存储 (T003→T004 [P] → T005→T006 [P] → T007? → T008→T009 [P] → T010 删除链+sqlc → T011 规则)
       └─ 模块 (T012,T013,T014 先写 [P] → T015 → T016 → T017 先写 → T018 → T019 先写 → T020 → T021)
...
**T012/T013/T014/T017/T019/T022/T023 必须先写并确认失败。**
