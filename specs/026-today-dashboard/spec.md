...
- **FR-005a**: §2 五项之后 MUST 呈现**两个附加区块**，顺序为：⑥账号配置缺项 ⑦待整理收件箱（修订 B，裁决 Q1 = A）。两者的标题 MUST 写其本义，MUST NOT 使用「待补录的反馈」或任何会让它被误读为 §2 第五项的标题。附加区块 MUST 排在五项之后，MUST NOT 插进五项之间。
- **FR-005b**: 「待补录的反馈」区块 MUST 列出**已发布、却一条人工指标都没录**的发布记录，数据源是 `feedback-learning` 的 `GET /api/content-feedback/pending`，派生口径用 `packages/core/content/feedback-learning` 的 `pendingFeedbackSummary`。每条 MUST 写明是哪一条（渠道 + 发布时间），MUST NOT 只显示一个数字。MUST NOT 隐藏该区块、MUST NOT 停在加载态、MUST NOT 显示任何伪造的反馈条目。
  ...
  **这个区块没有「到期」的概念**：判定只有「发了」和「一条都没录」两件事，不含任何时间比较。SOP §3.2 的品牌级「反馈观察时点」尚未落地（属 §3.2 那张卡），所以「过没过」今天无从判断；写一个默认天数等于发明一条 SOP 没说的规则，摆在运营者看不见也改不了的地方。
  ...
  > **2026-09-20 更新（specs/029 实施 PR 3，Issue #197）**：上一段的前提已经不成立——§3.2 的「反馈观察时点」落地了，天数**读自品牌设置**。本区块现在只列**观察天数已过**的发布记录；**没设过天数、或发布记录没有发布时间**的仍然在列，并在行上写明「答不出来」。上一段不删，因为它记的是当时为什么不发明一个默认天数——那条理由今天依然成立：天数来自运营者能看见也能改的设置，代码里**仍然没有**任何写死的天数。
...
- **[修订 B](./revision-b.md)**（Issue #188，裁决已回写）：接入 source-inbox 的快速采集入口与「待整理」区块（§11「随时」行）、侧栏「内容」分组（S1，五个入口）、默认落地页暂不改。
  - **FR-005a 据此修改**：附加区块从一个变**两个**（账号配置缺项、待整理收件箱），见下。
  - 修订 B 还报告了一个**已合入的缺陷**（Issue #191）：本卡交付的今日工作台上 12 处跳转用 workspace id 拼路径，而路由段是 slug，全部落空。**按裁决在修订 B 的实施之前单独修。**
