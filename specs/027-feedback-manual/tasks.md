...
- [x] T031 `packages/views/content/feedback-learning/`：指标与摘录区块，**只挂既有组件**，不新增控件、不改样式
- [x] T032 指标区：十项字段的表单；**`value` 留空与填 0 在界面上看得出来是两件事**（留空显示「未知」，不是显示 0，也不是空白让人猜）；十一个指标名的下拉；单位 / 窗口 / 证据是自由输入
- [x] T032a 粘贴 CSV：一个文本域 + 预览 + 「全有或全无」的提示；坏行**点名第几行的哪一列**；**没有文件上传**（等 W-03），并写明这一点
- [x] T033 摘录区：**摘录与解释是两个输入框**，各自可空；来源三选一；标签自由；**写明「脱敏请自己做，系统不会替你脱」**
- [x] T034 待补录区：列出已发布且无指标的发布记录，可直接跳去补录；**写明判定口径**（发了但一条指标都没录），不写「到期」——首版没有时间逻辑
- [x] T034a **AI 复盘占位**：显示 `pending_data`「待运行」，写明「暂不可用（执行器禁用，EP-08 接入）」；**没有任何伪造的复盘结论**
- [x] T035 接上今日工作台第五项（FR-025）+ `/{slug}/today` 到本区块的跳转（FR-026）
- [x] T036 本卡区块挂在 **025 发布记录那一段之后**：在 `review-delivery` 的页面组件上加一个**可选 render 插槽**，由 web 适配器注入。**不改登记表**（与 #161 对 `work-editor` 做的完全一样）
- [x] T037 四语言文案（en / zh-Hans / ja / ko）并跑 `locales/parity.test.ts`；新建 `manual-ui-todo.md`，界面项全部进去并一律记「未执行」；**不写 UI 单测**（宪法 II），核对 `packages/views` 下没有本卡新增的 `.test.tsx`
...
分支 `claude/impl-027-feedback-page`，base `app-main` @ `5b88b47`。T031–T037 全部落地，另加一处 PR 1 留下的修补。

**先说 PR 1 留下的一处错**：#177 给 `packages/views/package.json` 加了 `./content/feedback-learning` 导出，但那时没有 `index.tsx`，`rich-content/package-exports.test.ts` 因此在 `app-main` 上红了一轮，由 #182 单列提交删掉那一行。本 PR 把导出连同 `index.tsx` 一起加回，并把 views 的三套用例（`content` / `locales` / `rich-content`）纳入验证清单——**PR 1 只跑了 `packages/core` 的 vitest，这是那次漏掉的那一步。**

**四处与清单写的不一样，都在 PR 正文单列：**

1. **多了一个 `form.ts`（+ 45 条 node 用例）。** 清单里 T032 / T032a 只说「表单」「预览」，没说判定放哪。放进 core 的理由和 028 的 `draft.ts` 一样：按钮可不可点是规则，不是排版；而且它要和 Go 的三个上限对齐，`form.test.ts` 直接读 `contract.go` 比对 `MaxNoteRunes` / `MaxShortRunes` / `MaxTags`，桌面端将来复用的也是这些纯函数而不是这个页面。
2. **发布记录下拉是我加的。** 规格说指标绑一条发布记录（Assumptions 2：发了又删又发各自独立），但没说界面怎么选。默认选最新那条，可以切——不给选择的话，一篇有两条发布记录时录进去的数会悄悄挂在其中一条上。
3. **待补录里属于别的作品的行不给「去补这一条」按钮。** `listContentFeedbackPending` 是品牌全量，而这个区块是一篇文档的。第一版我给每行都挂了按钮，点了之后 `records.find` 落空会回退到本篇最新那条——等于把表单指到别处去。改成：不属于本篇的行不显示按钮，写明「在另一篇上」。新增手验项 027-U-36a。
4. **今日工作台第五项用 `SECTION_LIMIT`（10），不是 `pending.ts` 自己的 `DASHBOARD_PREVIEW`（5）。** `pendingFeedbackSummary` 的预览条数是参数，就是留给调用方定的。第五块停在 5 而邻居停在 10，看起来会像它数据少，而不是像它上限不同。

**一处没做「先写测试、确认失败」：** 和 PR 1 一样，`form.ts` 与 `form.test.ts` 是一起写的。页面 PR 没有变异验证这一环（宪法 II 不写 UI 单测），所以这次连替代证据也没有——如实记在这里。能说的只有：45 条 node 用例里，`parseValueInput` 的「空 ≠ 0」、`isTimestamp` 拒掉 `Date.parse` 会放行的三种写法、CSV 全有或全无这三组，是照着已知会出错的地方写的，不是照着实现反推的。
