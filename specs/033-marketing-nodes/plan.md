...
**主控前置决定**：2026-09-25 派单 8 条，已编码进 spec 的 FR。**待裁决**：Q1/Q2/Q3，本计划按推荐值 A 写；另有下方「主控决定」4 条需要主控拍板。

---

## 主控决定（本卡不能自己做的决定）

### D1 `adapters` 加两行（`modules` 不动）
...
**主控不批准时的退路**：处理函数写进已在册的 `content_topic.go`，页面做成 `/topics` 页里的一个分节（`topics/page.tsx` 已在册）。两条退路都可行，代价是 `content_topic.go` 变长、选题页多一块。**推荐批准两行**。

**谁来改**：推荐由实施 PR 1 带上第一行、PR 3 带上第二行，主控审查时对照本节；或者主控先单独提交这两行。二选一由主控定。

### D2 节点放在 `topic-planning`，不新建模块
...
### D3 开始快照不加「节点版本」扩展字段（推荐）

文档 14 说「运行快照引用确定版本」。今天没有「运行」（`agent-workflow` 未落地），只有选题卡「开始」时冻结的 `StoredSnapshot`（`topic-planning/snapshot.go:45-57`）。

- **推荐：本卡不加**。可追溯性由「卡 → 候选 → `adopted_revision` → 只插的版本表」提供，改 `snapshot.go` 会碰 023 的守卫文件，且每次开始都要多一次按卡反查候选。
- 备选：照 030 的做法给 `StoredSnapshot` 加第五个扩展字段 `marketing_node_revisions`（数组：`{node_id, revision}`），放进 PR 2。023 的 `TestNoSourcelessFieldIsInvented` 一行不改。

### D4 预填到 `timing` 的那段文字用哪种语言（小点）

采用时 `timing` 预填一段由字段拼成的事实（contract §4.3）。服务端只存一种写法。**推荐中文固定模板**：节点名、账号都是中文，卡正文也都是中文；做成四语言需要服务端知道用户语言，这在 content 模块里还没有先例。
...
**存储**：PostgreSQL 17。3 张新表、6 个并发索引，迁移 540–548（若被占用整体顺延）。无外键、无级联、无内联主键 / 唯一约束。
...
**Stop Conditions**：constitution 可读；权威来源可读；文件范围见下方清单；**唯一越出既有登记的是 `adapters` 两行，已按主控要求交给主控决定（D1），没有靠削弱任何规则通过**。
...
```
server/migrations/540_content_marketing_node.{up,down}.sql
server/migrations/541_content_marketing_node_id_unique_idx.{up,down}.sql
server/migrations/542_content_marketing_node_workspace_idx.{up,down}.sql
server/migrations/543_content_marketing_node_revision.{up,down}.sql
server/migrations/544_content_marketing_node_revision_id_unique_idx.{up,down}.sql
server/migrations/545_content_marketing_node_revision_node_revision_idx.{up,down}.sql
server/migrations/546_content_marketing_node_candidate.{up,down}.sql
server/migrations/547_content_marketing_node_candidate_id_unique_idx.{up,down}.sql
server/migrations/548_content_marketing_node_candidate_key_idx.{up,down}.sql
...
- `scripts/content-boundaries.json`（D1 由主控处理）
- `content_topic_card` 表结构、`TopicCard` 契约（不加列、不加字段）
- `content_brief_revision`、`content_start_snapshot` 的任何语句；`snapshot.go`（D3 推荐不加）
...
| | 内容 | 不含 | 验收 |
|---|---|---|---|
| **PR 1** | 迁移 540–548、删除链、节点的新建 / 导入 / 修改 / 确认 / 取消 / 读 / 历史、日期与阶段、core 解析与 CSV | 候选、采用、影响、页面 | D14-V01（节点部分）、D14-V02（日期部分）、D14-V03（版本只插） |
...
| 风险 | 为什么真实 | 对策 |
|---|---|---|
| **抽出 `Create` 的 INSERT 时改了 `Create` 的行为** | 抽函数最容易顺手把栅栏外的账号校验也挪进来——那是「顺手修」（宪法 VIII） | PR 2 只移动 INSERT 与素材校验到一个接收 `pgx.Tx` 的函数；`Create` 的既有用例全部一行不改且绿；账号校验位置写进 Out of Scope 10 |
...
| **重复整理产生重复候选** | 两个请求并发时应用层的「先查再插」会各插一条 | 548 唯一索引 + `ON CONFLICT DO NOTHING`；并发用例；变异 M4 |
