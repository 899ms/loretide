...
状态：全部 TODO。关联 [模块边界约束](../docs/12-模块边界与变更回归约束.md) 与 [主任务清单](todo.md)。这里保存两项新增前置任务的唯一状态；主清单只引用，不重复勾选。

## ARCH-01 · 定义可执行的模块依赖清单
- [ ] **TODO**；依赖：LT-001、LT-002；规模：S；关联：NFR-03、任务计划共同完成标准。
...
## ARCH-02 · 将边界检查接入开发验证
- [ ] **TODO**；依赖：ARCH-01、LT-007；规模：M；关联：NFR-03、D10-V01、docs/12。
